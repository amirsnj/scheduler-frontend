    ...
    <div class="max-w-md w-full space-y-8">
      ...
      <div class="text-center">
        ...
        <h2 class="text-3xl font-bold text-gray-900 mb-2">
          {{ locales[currentLanguage].createAccount }}
        </h2>
        <p class="text-gray-600">
          {{ locales[currentLanguage].getStarted }}
        </p>
      </div>
      ...
      <div class="bg-white rounded-xl shadow-lg p-8">
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-2 gap-4">
            <!-- First Name -->
            <div>
              <label for="firstName" class="block text-sm font-medium text-gray-700 mb-2" :class="{ 'text-right': currentLanguage === 'fa' }">
                {{ locales[currentLanguage].firstName }}
              </label>
              <div class="relative">
                ...
                <input
                  ...
                  :placeholder="locales[currentLanguage].firstName"
                />
              </div>
              <p v-if="errors.firstName" class="mt-1 text-sm text-red-600" :class="{ 'text-right': currentLanguage === 'fa' }">{{ errors.firstName }}</p>
            </div>

            <!-- Last Name -->
            <div>
              <label for="lastName" class="block text-sm font-medium text-gray-700 mb-2" :class="{ 'text-right': currentLanguage === 'fa' }">
                {{ locales[currentLanguage].lastName }}
              </label>
              <div class="relative">
                <div class="absolute inset-y-0 flex items-center pointer-events-none" :class="currentLanguage === 'fa' ? 'right-0 pr-3' : 'left-0 pl-3'">
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-gray-400">
                ...
                <input
                  ...
                  :placeholder="locales[currentLanguage].lastName"
                />
              </div>
              <p v-if="errors.lastName" class="mt-1 text-sm text-red-600" :class="{ 'text-right': currentLanguage === 'fa' }">{{ errors.lastName }}</p>
            </div>
          </div>
          ...
          <!-- Email -->
          <div>
            <label for="email" class="block text-sm font-medium text-gray-700 mb-2" :class="{ 'text-right': currentLanguage === 'fa' }">
              {{ locales[currentLanguage].email }}
            </label>
            <div class="relative">
              <div class="absolute inset-y-0 flex items-center pointer-events-none" :class="currentLanguage === 'fa' ? 'right-0 pr-3' : 'left-0 pl-3'">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-gray-400">
              ...
              <input
                ...
                :placeholder="locales[currentLanguage].email"
              />
            </div>
            <p v-if="errors.email" class="mt-1 text-sm text-red-600" :class="{ 'text-right': currentLanguage === 'fa' }">{{ errors.email }}</p>
          </div>
          ...
          <!-- Confirm Password -->
          <div>
            <label for="confirmPassword" class="block text-sm font-medium text-gray-700 mb-2" :class="{ 'text-right': currentLanguage === 'fa' }">
              {{ locales[currentLanguage].confirmPassword }}
            </label>
            <div class="relative">
              <div class="absolute inset-y-0 flex items-center pointer-events-none" :class="currentLanguage === 'fa' ? 'right-0 pr-3' : 'left-0 pl-3'">
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-gray-400">
              ...
              <input
                ...
                :placeholder="locales[currentLanguage].confirmPassword"
              />
              <button
                type="button"
                ...
                class="absolute inset-y-0 flex items-center" :class="currentLanguage === 'fa' ? 'left-0 pl-3' : 'right-0 pr-3'"
              >
                <svg v-if="showConfirmPassword" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-gray-400">
                  <path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/>
                  <circle cx="12" cy="12" r="3"/>
                </svg>
                <svg v-else width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" class="text-gray-400">
            ...
            <p v-if="errors.confirmPassword" class="mt-1 text-sm text-red-600" :class="{ 'text-right': currentLanguage === 'fa' }">{{ errors.confirmPassword }}</p>
          </div>
          ...
          <div class="flex items-center" :class="{ 'flex-row-reverse': currentLanguage === 'fa' }">
            <input
              ...
              :class="[
                ...
                { 'ml-2': currentLanguage === 'fa', 'mr-2': currentLanguage === 'en' }
              ]"
            />
            <label for="acceptTerms" class="block text-sm text-gray-700" :class="{ 'mr-2': currentLanguage === 'fa', 'ml-2': currentLanguage === 'en' }">
              {{ locales[currentLanguage].acceptTerms }}
            </label>
          </div>
          <p v-if="errors.acceptTerms" class="mt-1 text-sm text-red-600" :class="{ 'text-right': currentLanguage === 'fa' }">{{ errors.acceptTerms }}</p>
          ...
          <div>
            ...
            >
              ...
              {{ locales[currentLanguage].signUpButton }}
            </button>
          </div>
...
const validateForm = (): boolean => {
  ...
  if (!formData.firstName.trim()) {
    newErrors.firstName = locales[currentLanguage.value].firstNameRequired;
  }

  // Last name validation
  if (!formData.lastName.trim()) {
    newErrors.lastName = locales[currentLanguage.value].lastNameRequired;
  }
  ...
  if (!formData.email) {
    newErrors.email = locales[currentLanguage.value].emailRequired;
  } else if (!emailRegex.test(formData.email)) {
    newErrors.email = locales[currentLanguage.value].emailInvalid;
  }
  ...
  if (!formData.confirmPassword) {
    newErrors.confirmPassword = locales[currentLanguage.value].confirmPasswordRequired;
  } else if (formData.password !== formData.confirmPassword) {
    newErrors.confirmPassword = locales[currentLanguage.value].passwordMismatch;
  }

  // Accept terms validation
  if (!formData.acceptTerms) {
    newErrors.acceptTerms = locales[currentLanguage.value].acceptTermsRequired;
  }
...
const handleSubmit = async (): Promise<void> => {
  ...
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      notificationStore.showSuccess(locales[currentLanguage.value].registerSuccess);
