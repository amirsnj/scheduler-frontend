<template>
  <div class="w-72 h-screen bg-white rounded-xl p-5 flex flex-col font-sans shadow-lg transition-transform duration-300 ease-in-out scrollbar-hide overflow-y-auto">
    <!-- Header -->
    <div class="mb-5">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span class="text-base font-semibold text-gray-800">{{ locales[currentLanguage].menu }}</span>
        </div>
    ...
    <!-- Tasks Section -->
    <div class="mb-8">
      <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 ml-1">{{ locales[currentLanguage].tasks }}</h3>
      <ul class="space-y-0.5">
        ...
        >
          ...
          <span class="flex-1 text-sm font-medium">{{ locales[currentLanguage].upcoming }}</span>
          <span class="text-xs font-medium" :class="activeItem === 'upcoming' ? 'text-blue-600' : 'text-gray-400'">{{ taskStore.upcomingTasks.length }}</span>
        </li>
        <li 
          class="flex items-center px-3 py-2 rounded-lg cursor-pointer transition-all text-gray-500 hover:bg-gray-100 hover:text-gray-700"
        ...
        >
          ...
          <span class="flex-1 text-sm font-medium">{{ locales[currentLanguage].today }}</span>
          <span class="text-xs font-medium" :class="activeItem === 'today' ? 'text-blue-600' : 'text-gray-400'">{{ taskStore.todayTasks.length }}</span>
        </li>
        <li 
          class="flex items-center px-3 py-2 rounded-lg cursor-pointer transition-all text-gray-500 hover:bg-gray-100 hover:text-gray-700"
        ...
        >
          ...
          <span class="flex-1 text-sm font-medium">{{ locales[currentLanguage].completed }}</span>
          <span class="text-xs font-medium" :class="activeItem === 'completed' ? 'text-blue-600' : 'text-gray-400'">{{ taskStore.completedTasks.length }}</span>
        </li>
        <li 
          class="flex items-center px-3 py-2 rounded-lg cursor-pointer transition-all text-gray-500 hover:bg-gray-100 hover:text-gray-700"
        ...
        >
          ...
          <span class="flex-1 text-sm font-medium">{{ locales[currentLanguage].calendar }}</span>
        </li>
      </ul>
    </div>
    ...
    <!-- Lists Section -->
    <div class="mb-8">
      <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 ml-1">{{ locales[currentLanguage].lists }}</h3>
      <ul class="space-y-0.5">
        <li 
          ...
          :class="{ 'bg-blue-50 text-blue-600': activeItem === `category-${list.id}` }"
          @click="setActive(`category-${list.id}`)"
        >
          <div class="w-3.5 h-3.5 rounded-sm mr-3 flex-shrink-0" :class="getListColor(list.id)"></div>
          <span class="flex-1 text-sm font-medium">{{ list.title }}</span>
          <span class="text-xs font-medium" :class="activeItem === `category-${list.id}` ? 'text-blue-600' : 'text-gray-400'">{{ list.task_count }}</span>
        </li>
      </ul>
    ...
    <!-- Tags Section -->
    <div class="mb-8">
      <h3 class="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3 ml-1">{{ locales[currentLanguage].tags }}</h3>
